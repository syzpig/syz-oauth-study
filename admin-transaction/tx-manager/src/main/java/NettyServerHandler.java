...
public class NettyServerHandler extends ChannelInboundHandlerAdapter{
    ...
    /**
     *创建事务组，并添加保存事务
     * 并且需要判断，如果所有的事务都已经执行了（有结果了，要么回滚，要么提交），且其中有一个事务需要回滚，
     * 那么通知所有的客户端进行回滚，否则，则通知所有客户端进行提交
     *
     */
    public synchronized void channelRead(ChannelHandlerContext ctx,Object msg) throws Exception{
       ...
       String transactionType=jsonObject.getString("transactionType");//子事务类型，commit-待提交、rollback-待会滚
       Integer transactionCount=jsonObject.getInteger("transactionType");//事务数量
       Boolean isEnd=jsonObject.getBoolean("isEnd");//是否是结束事务
        ...
        if("create".equals(command)){
            ...
            //如果已经收到结束事务的标记，比较事务是否已经全部到达，如果已经全部到达则看是否需要回滚
